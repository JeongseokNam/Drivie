...
<script type="text/javascript">
	function deleteChk(no) {
		if (!confirm("정말 삭제하시겠습니까?")) {
			...
			location.href = 'admin_delMnT?no=' +no;
		}
	}
</script>
</head>
<body>
	...
	<div class="container">
		<div class="row mt-5">
			<div class="col-6">
				<form action="admin_searchMnT" method="get">
		...
		<div class="row mt-3">
			<table class="table col">
				<thead class="table-dark">
					<tr>
						<th class="col" scope="col">포스터</th>
						<th class="col" scope="col">영화관</th>
						<th class="col" scope="col">영화이름</th>
						<th class="col" scope="col">날짜/시간</th>
						<th class="col-1" scope="col">삭제</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${MnTList }" var="MnTList">
						<tr>
							<th style="width: 200px; height: 300px;"><img
								src="resources/tmp/${MnTList.movieVO.photo }" id="img" alt="미리보기"
								style="width: 200px; height: 300px;"></th>
							<td>${MnTList.theaterVO.theatername }</td>
							<td>${MnTList.movieVO.title }</td>
							<td>${MnTList.showtime }</td>
							<td>
								<button type="button" class="btn btn-dark"
									onclick="deleteChk('${MnTList.no }')">삭제</button>
							</td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<div>
				<form action="admin_addMovieInTheater" method="post">
					<div class="input-group mb-3">
					<select class="form-select form-select-sm" name="theater_no">
						...
						<select class="form-select form-select-sm" name="movie_no">
							<option selected>영화선택</option>
							<c:forEach items="${movieList }" var="movieList">
							<option value=${movieList.no }>${movieList.title }</option>
							</c:forEach>
						</select>
